...
Context.prototype.parse = function (content,info) {
    this.parser.parse(content, {ctx:info && info.ctx || this, filename:this.filename});
    return this;
}
...
Context.prototype.parseTagContent = function parseTagContent(buffer, info, filename) {
    var tcx = this.tagContent = new Context(null, this, this.resolver, (filename || info.filename)+'#tagContent');
...
Context.prototype.parseTag = function (prefix, tag, attr, buffer, info) {
    ...
    var tagCtx = this.parseTagContent(buffer, info, prefix+'/'+tag+"#tagContent");
    var tagFuncWrap = function (tagFunc) {

        return function (attr, p, scope) {
            return   tagFunc.call(this, attr, tagCtx, scope, info);
        }
    }
    if (tagFunc) {
        ...
        this.content.push('<%= ' + info.ctx._scope(tagFuncWrap(tagFunc)(attr, info.ctx.tagContent, this), 'tagScope', '$' + (_scopeCount++), prefix, tag) + '(this, arguments) %>');
        return this;
    }
    var d = Q.defer();
    this.content.push(d.promise);
    this.resolver(taglib, tag).then(function (tagContent) {
        var tagO = new Tag(taglib, tag, new Context(null, null, this.resolver,  taglib + '/' + tag).parse(tagContent, info));
        tagO.context.isTag = true;
        tagO.create().then(function onTagCreateExec(tagFunc) {
            (ParsedTags[taglib] || (ParsedTags[taglib] = {}))[tag] = tagFunc;
            d.resolve('<%= ' + info.ctx._scope(tagFuncWrap(tagFunc)(attr, info.ctx.tagContent, this), 'tagScope', '$' + (_scopeCount++), prefix, tag) + '(this, arguments) %>');
        }.bind(this))
    }.bind(this));
}
