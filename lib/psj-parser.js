...
//                    i = whitespace_swallow(str, i);
        ...
        this.parseBody(str.substring(i), _.extend({ctx: context}, info, {filename: filename, line: countLines(str, i), chr: i}));
    }

    this.parseBody = function parseBody(str, info) {
        ...
        for (var i = 0, n = 1, l = str.length; i < l; i++, n = i + 1) {
            if (state === States.TAG_END) {
                ...
                state = 0;
                pt = nprefix = ntag = prefix = attr = tag = buffer = "";
                continue;
            }
            var chr = str[i], next = str[n];
