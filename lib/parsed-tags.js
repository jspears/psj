...
define(['./psj-eval-expression', 'underscore'], function (EvalExpr, _) {
    return  {
        'http://java.sun.com/JSP/Page': {
            doBody: function onValidateDoBody(attr, tagContent, info) {
                ...
                return function onEvalDoBody(ctx, scope) {
                    ...
                    try {
                        //TODO - I think this is a mistake, what I _think_ is the right answer is to change
                        // the _scope(tagScope bits so that when it is a nested tag, it takes a 3rd argument.
...
//        },
            'set': function onSet(attr, parent) {
                var valueExpr = 'value' in attr ? EvalExpr.expression(parent, attr.value) : parent && parent.scope && parent.scope.contentScope && parent.scope.contentScope._template();
                var scopeName = (attr.scope || 'page') + 'Scope';
                var varName = attr.var;
                return function onSetExec(ctx, scope) {
                    var val = valueExpr()(scope)(scope);
                    if (attr.target && attr.property)
                        scope[scopeName][attr.target][attr.property] = val;
                    if (varName)
                       this[varName] = scope[scopeName][varName] = val;
                }
