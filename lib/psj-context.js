...
Context.prototype.parseTag = function (prefix, tag, attr, buffer) {
    ...
    this.resolver(taglib, tag).then(function (value) {
        var tag = ParsedTags[taglib] && (ParsedTags[taglib][tag] = new Tag(taglib, tag, value)).create();
//        var ctx = new Context(null, this, this.resolver);
        content.push(function(){
                tag(attr, buffer);
            //this._scope(tag.create.bind(tag), 'pageScope', prefix, tag) + '(evalScope.attrs, ' +JSON.stringify(buffer)+');'
        });
...
Context.prototype.parseEvalOut = function (content, char, line) {
    return this.content.push(function(){ return content; });
}
