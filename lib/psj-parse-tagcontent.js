...
define(['./text-util', 'underscore'], function (tu, _) {
    var whitespace = tu.whitespace,
...
//                    i += 2;
                    ...
                    if (chr === '@')
                        throw createError("Can not have tag directives in the body", str, i, info);
                ...
                if (state === States.TAG_START) {
                    if (chr === ':') {
                        if (ntag in taglibs) {
                            var pc = buffer.substring(0, (buffer.length - ntag.length - 1));
                            if (pc) {
                                parseContent(pc);
                                buffer = "";
                            }
