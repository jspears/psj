...
//           ctx.parse(core+'<c:if test="${myitems}">hello</c:if>')
    ...
    'test set tag': function (test) {
        ...
        ctx.render({myitems: 1}, function (err, out) {
    ...
    'choose-when-otherwise': function (test) {
        try {
            var ctx = new Context(null, null, resolver, 'test#choose-when-otherwise');
            ctx.parse('<%@taglib prefix="core"  uri="http://java.sun.com/jsp/jstl/core" %><core:choose><core:when test="${empty myitems}">hello</core:when><core:when test="${not empty myitems}">say</core:when><core:otherwise>goodbye</core:otherwise></core:choose>');
            ctx.render({myitems: 'abcd'.split('')}, function (err, out) {
                out = out.replace(/\s+/g, ' ').trim();
